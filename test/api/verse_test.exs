defmodule DigitalBibleVerseTest do
  ...
  describe "When passing a chapter model" do
    ...
    test "it makes Verse models", state do
      first_result = List.first(state[:verse_list])
      assert first_result == %DigitalBible.Model.Verse{
        verse_id: "1", book_id: "Matt", book_name: "Matthew", book_order: "55", chapter_id: "3", chapter_title: "Chapter 3", paragraph_number: "3", verse_text: "Now in those days John the Baptist came, preaching in the wilderness of Judea, saying, \n\t\t\t"
      }
    end
  end
